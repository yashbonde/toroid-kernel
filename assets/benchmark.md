...
repo (`kernel.go`, `tools/`, `provider.go`, `events.go`).

| Feature | toroid | Claude Code | Claude Agent SDK | OpenAI Codex CLI | pi.dev |
|---|---|---|---|---|---|
| **Distribution** | Go library, embed in your own program | Standalone CLI / IDE ext / desktop app | Go/Python/TS SDK, embed in your own program | Standalone CLI (Rust) / IDE ext / desktop app / cloud | Standalone CLI + TUI, npm/git packages |
...
| **Built-in tools** | Bash, Read, Write, Edit, MultiEdit, Glob, Grep, LS, Todo, Notify, Subagent (`tools/`) | Bash, Read, Write, Edit, Glob, Grep, WebFetch, WebSearch, Task/Agent, TodoWrite, NotebookEdit, etc. | Same tool surface as Claude Code, programmatically composable | Bash/exec, file read/write/patch, web search (cloud mode) | Deliberately just 4: read, write, edit, bash — everything else is opt-in |
| **MCP client support** | **None today** — tool registry (`tools/registry.go`) is hand-registered `fantasy.AgentTool`s only | Yes — full MCP client (stdio/SSE), first-class | Yes — inherits Claude Code's MCP client | Yes — MCP client support | **None by design** — "MCP is overkill"; favors CLI tools + README-based progressive disclosure over MCP |
| **Skills / progressive-disclosure capability packages** | **None** — no on-demand capability loading; all tools are always in the registry | Yes — Skills marketplace, `/skill:name`, loaded on demand, composes with hooks | Yes (same primitive, programmatic) | Partial — custom instructions/prompts, no first-class skills marketplace | Yes — first-class Skills concept, invoked via `/skill:name`, explicitly built for progressive disclosure without busting prompt cache |
...
| **Extensibility model** | Go package — extend by writing Go code against `tools.Registry` | Skills, hooks, MCP, plugins, marketplace | Full programmatic control — build any of the above yourself | Extensions via SDK, GitHub/Slack integrations | Packages (extensions, skills, prompts, themes) via npm/git |
...
Persistence & Telemetry (arguably ahead of everyone else there), but at zero
on MCP and Skills, and weak on Permission/Sandbox — the same three gaps
called out below.

### Biggest gaps for toroid, in priority order

1. **MCP client** — no way to talk to external tool servers today; this is
   the most-requested interoperability feature and the most mechanical to
   add given `tools.Registry` is already tool-agnostic.
2. **Skills / progressive disclosure** — no on-demand capability loading;
   every tool the kernel knows about is always resident. Both Claude Code
   and pi.dev treat this as a first-class primitive for keeping prompts
   cache-friendly as capability surface grows.
