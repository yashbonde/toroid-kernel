...
func TransformForHandoff(history []fantasy.Message, from, to Model) []fantasy.Message {
	if from.API == to.API {
		return history // same wire: nothing to transform (the in-scope path)
	}
	out := make([]fantasy.Message, 0, len(history))
...
func stripReasoning(msg fantasy.Message) fantasy.Message {
	filtered := make([]fantasy.MessagePart, 0, len(msg.Content))
	for _, part := range msg.Content {
		if _, isReasoning := fantasy.AsMessagePart[fantasy.ReasoningPart](part); isReasoning {
